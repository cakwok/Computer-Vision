'''
Wing Man Casca, Kwok
CS5330 Project 6 - GAN MNIST dataset extensions - Latent size experiment
'''
import torch
...
class Discriminator(torch.nn.Module):               #define a discriminator NN
    def __init__(self):
...
class Generator(nn.Module):                     #define a Generator NN
    def __init__(self):
...
def GenerateLatentVector(size):     #generate a latent vector
    return torch.randn(size,144)    #randn produces a tensor with gaussian, zero mean, variance 1

def ones_target(size):              #generate all one tensor
    return torch.ones(size,1)

def zeros_target(size):             #generate all zero tensor

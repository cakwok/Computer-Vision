'''
Wing Man Casca, Kwok
CS5330 Project 6 - GAN MNIST dataset extensions - Batch size experiment
'''
import torch
...
def ones_target(size):              #generate an all 1 tensor
    return torch.ones(size,1)

def zeros_target(size):             #generatet a all zero tensor
...
def main(argv):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 20 == 0:             #check status of every 20 epochs

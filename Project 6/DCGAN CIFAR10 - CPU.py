...
def GenerateLatentVector(size):
    return torch.randn(size,100, 1, 1)    #produces a tensor with gaussian, zero mean, variance 1
...
def main(argv):
    ...
    for epoch in range(num_epochs):
        for n_batch, (real_batch, example_targets) in enumerate(data_loader):
            ...
            latent_data = generator(torch.randn(batch_size,100, 1, 1)).detach()
    ...
    #print("generator(test_noise).shape", generator(test_noise).shape)      #output: torch.Size([16, 3, 64, 64])
    ...
    for i in range(6):
        ...
        plt.axis("off")
        plt.imshow(np.transpose(test_images[i].detach().numpy(), (1,2,0)))
